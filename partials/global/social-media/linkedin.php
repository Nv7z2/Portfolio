<?php

$linkedin_link = get_field('linkedin', 'option');

?>

<a href="<?php echo $linkedin_link; ?>" target="_blank" title="kacper baran linkedin">
    <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 35 35" fill="none" class="social-media-icon">
    <g clip-path="url(#clip0_248_222)">
        <path d="M29.6528 1.45831L5.34726 1.45831C3.19949 1.45831 1.45837 3.19943 1.45837 5.3472L1.45837 29.6528C1.45837 31.8005 3.19949 33.5416 5.34726 33.5416L29.6528 33.5416C31.8006 33.5416 33.5417 31.8005 33.5417 29.6528L33.5417 5.3472C33.5417 3.19943 31.8006 1.45831 29.6528 1.45831Z" stroke="#262626" stroke-width="2"/>
        <path d="M10.2084 15.0695L10.2084 25.764" stroke="#262626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        <path d="M10 11C10.5523 11 11 10.5523 11 10C11 9.44772 10.5523 9 10 9C9.44772 9 9 9.44772 9 10C9 10.5523 9.44772 11 10 11Z" fill="#262626"/>
        <path d="M24.7916 25.764V19.4445C24.7916 17.0283 22.8328 15.0695 20.4166 15.0695C18.0004 15.0695 16.0416 17.0283 16.0416 19.4445V25.764" stroke="#262626" stroke-width="2" stroke-linecap="round"/>
    </g>
    <defs>
        <clipPath id="clip0_248_222">
        <rect width="35" height="35" fill="white"/>
        </clipPath>
    </defs>
    </svg>
</a>